...
void v8_spi_master_clk_config(uint32_t clk_rate);
void v8_spi_master_config_control_data(uint8_t bit, uint8_t on);
...
// **********************************************************************
SPI_MasterSlave::SPI_MasterSlave(void)
{
	SPI_MasterSlave(SPI_MASTER);
};

SPI_MasterSlave::SPI_MasterSlave(uint8_t type)
{
	_spiType = type;
	_spiMode = 0;
...
// **********************************************************************
void SPI_MasterSlave::config(uint8_t spiMode, uint32_t clkRate, bool en_cs1, bool en_cs2)
{
...
// **********************************************************************
void SPI_MasterSlave::begin(void)
{
...
void SPI_MasterSlave::setClock(uint32_t clkRate)
{
   if (_spiType == SPI_MASTER) {
   v8_spi_master_clk_config(clkRate);
   }
}
...
// Description:
// **********************************************************************
void SPI_MasterSlave::resetTx(void)
{
	txdSize = 0;
	txdPtr = 0;
}

void SPI_MasterSlave::resetRx(void)
{
	rxdSize = 0;
	rxdPtr = 0;
}
...
// **********************************************************************
size_t SPI_MasterSlave::write(uint8_t data)
{
...
size_t SPI_MasterSlave::write(uint8_t *data, size_t size)
{
	uint16_t sz, k;
...
// **********************************************************************
void SPI_MasterSlave::slaveSelect(uint8_t slv){
...
// **********************************************************************
size_t SPI_MasterSlave::transfer(void)
{
	uint16_t io_num;
...
size_t SPI_MasterSlave::transfer(size_t size)
{
	txdSize = size;
	return transfer();
}
...
// **********************************************************************
int SPI_MasterSlave::available(void)
{
	return ((int)rxdSize);
}
...
// **********************************************************************
int SPI_MasterSlave::remaining(void)
{
	return ((int)txdSize);
}
...
// Description:
// **********************************************************************
uint8_t SPI_MasterSlave::pick(size_t offset)
{
	return rxd[offset];
}
...
// Description:
// **********************************************************************
void SPI_MasterSlave::enableBufferForHostWrite(void)
{
...
// Description:
// **********************************************************************
bool SPI_MasterSlave::validBufferForHostRead(void)
{
	return s2m_buffer_can_fill;
}

uint8_t SPI_MasterSlave::copyDataToBufferForHostRead(void)
{
	uint8_t s2m_num;
...
void SPI_MasterSlave::enableBufferForHostRead(void)
{
	if (_spiType == SPI_MASTER) return;
	s2m_buffer_can_fill = false;
...
void SPI_MasterSlave::disableBufferForHostRead()
{
	if (_spiType == SPI_MASTER) return;
	s2m_buffer_can_fill = true;
...
// Description:
// **********************************************************************
void SPI_MasterSlave::attachInterrupt(uint8_t type, void (*userFunc)(void))
{
	if (_spiType == SPI_MASTER) return;
...
void SPI_MasterSlave::detachInterrupt(uint8_t type)
{
	if (_spiType == SPI_MASTER) return;
...
// Description:
// **********************************************************************
void SPI_MasterSlave::isr(uint8_t type)
{
...
// Description:
// **********************************************************************
SPI_MasterSlave::operator bool() {
	return enabled;
}


// **********************************************************************
// Description: Instances of SPI master
// **********************************************************************
...
SPIClass::SPIClass(uint8_t type)
{
	_spiType = type;
	_spiMode = 0;
	_clkRate = 1000000; // default 1MHz
...
void SPIClass::begin(void)
{
	...
	//arduino way does not use 28 for digital write
	v8_spi_master_init(_clkRate, _en_slave_cs1, _en_slave_cs2);
	v8_spi_master_config(_spiMode, false); // no INTR now
		
	if(_spiLSB==1)
        v8_spi_master_config_control_data(11, 1);
        else
        v8_spi_master_config_control_data(11, 0);
...
uint8_t SPIClass::transfer(uint8_t data)
{
	...
	if (!initialized) { return 0x0; } // object disabled
	
	io_num = v8_spi_master_byte_io(_slave_cs, &data, &rxd, 1);
...
uint16_t SPIClass::transfer16(uint16_t data)
{
	...
	io_num = v8_spi_master_byte_io(_slave_cs, txd, rxd, 2);
...
void SPIClass::beginTransaction(SPISettings settings)
{
    ...
    _clkRate = settings._spi_clock;
    
    if (settings._spi_dataMode < 2) _spiMode = settings._spi_dataMode;
    if (settings._spi_bitorder < 2) _spiLSB = settings._spi_bitorder;
...
{
  //no need to do anything, since other libraries can use the SPI too after chip select is deassert
}
