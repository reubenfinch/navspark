...
#endif
	return SERIAL_RING_BUFFER_SIZE - xmit.numData;
}
...
#endif
	if (enabled == true) {
		if(xmit.numData == SERIAL_RING_BUFFER_SIZE) { emptyTx(); }
	  // disable interrupt for integrity
    gnss_disable_irq(UART_ISR_NUMBER);
...
void HardwareSerial::emptyTx(void)
{
	/*
	uint8_t watchdog_active = v8_watchdog_status();
	...
	do{i=ioreg[((port_id == NMEA_PORT_ID) ? 0x4f0c : 0x4f10)/4];}while((i >> 31) == 1);
