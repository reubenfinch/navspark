...
extern "C" {
#endif
void isrWireSlaveFunc(void);
#ifdef __cplusplus
}
#endif


uint8_t TwoWire::_mode = WIRE_MASTER;
...
TwoWire::TwoWire()
{
	...
	pRecv = (pTwoWireBuffer) &recv;
	pXmit = (pTwoWireBuffer) &xmit;
	// set buffer sizes
...
{
	Wire.reset();
	Wire.setTransmitDeviceAddr(address);
	Wire.setReceiveDeviceAddr(address);
}
...
void TwoWire::send(uint8_t *data, size_t size)
{
	write(data, size);
}
...
// **********************************************************************
size_t TwoWire::recv_space(void)
{
	return ((size_t)(pRecv->numSpace - pRecv->numData));
}
...
// **********************************************************************
size_t TwoWire::xmit_space(void)
{
	return ((size_t)(pXmit->numSpace - pXmit->numData));
}
...
void TwoWire::isr(void)
{
	...
	if(_mode == WIRE_MASTER) { return; }//not slave mode  
