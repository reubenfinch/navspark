...
class SPI_MasterSlave
{
	...
	public:
		SPI_MasterSlave(uint8_t type);
		SPI_MasterSlave(void);
		...
		//set SPI master clk
	        void setClock(uint32_t clkRate);
...
extern SPI_MasterSlave spiMaster;
extern SPI_MasterSlave spiSlave;
...
public:
	...
	init_SPIsetting(clock, bitOrder, dataMode);
	}
	SPISettings() { init_SPIsetting(5000000, MSBFIRST, SPI_MODE0); }
private:
     void init_SPIsetting(uint32_t clock, uint8_t bitOrder, uint8_t dataMode){
     _spi_clock=clock;
     _spi_bitorder = bitOrder;
     _spi_dataMode = dataMode;
     }
     uint32_t _spi_clock;
...
class SPIClass
{
	private:
		...
		uint8_t	_spiType;	// 1 for master, 0 for slave
		uint8_t _spiMode; // 0 for mode 0, 1 for mode 1
		bool _spiLSB;     // 0 for MSG, 1 for LSB
		uint32_t _clkRate; // valid in master mode
		bool _en_slave_cs1; // true to enable extra CS1
		bool _en_slave_cs2; // true to enable extra CS2
		uint8_t	_slave_cs; // 0 -> GPIO28, 1 -> GPIO22 and 2 -> GPIO6
