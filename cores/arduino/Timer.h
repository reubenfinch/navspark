...
class TIMER
{
	...
	public:
		...
		// functions for starting timer, period in unit of micro-second
		uint8_t everyMicro(uint32_t period, void (*callback)(void));
		uint8_t everyMicro(uint32_t period, void (*callback)(void), uint16_t repeatCount);
		// functions for starting timer, period in unit of nano-second
		uint8_t everyNano(uint32_t period, void (*callback)(void));
		uint8_t everyNano(uint32_t period, void (*callback)(void), uint16_t repeatCount);

		uint8_t after(uint32_t period, void (*callback)(void));
		uint8_t afterMicro(uint32_t period, void (*callback)(void));
		uint8_t afterNano(uint32_t period, void (*callback)(void));
